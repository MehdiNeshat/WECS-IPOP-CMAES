...
%  disp(['SEED RELOADED FROM ' opts.SaveFilename]);
  startseed = rng('shuffle');%randn('state');         % for retrieving in saved variables
...
str = [' (startseed=' num2str(startseed.State(2)) ...
              ', ' num2str(clock, '%d/%02d/%d %d:%d:%2.2f') ')'];
...
while isempty(stopflag)
  ...
  %%------------------------------------------------------------------      
        rep=0;
        while (1) % until generate feasible solution
        rep=rep+1;
      ...
      % control feasibility
      arx(:,k)          = xintobounds(arx(:,k), lbounds, ubounds);
      temp              = arx(:,k)';
      coordinate(1,:)   = temp(1:2:end);
      coordinate(2,:)   = temp(2:2:end);
      Temp_pos(rep).position=coordinate;
      [flag, violation(rep)] = Control_feasibility(coordinate);
      if flag==1
         break 
      end
      if rep >= 1000 % selecting the solution with minimum violation
          [MinVio MinIndex]=min(violation);
          coordinate       =  Temp_pos(MinIndex).position;
          arx(1:2:Opt.Buoy_Num*2,k)= coordinate(1,:);
          arx(2:2:Opt.Buoy_Num*2,k)= coordinate(2,:);
          clean Temp_pos;
          clean violation;
         break 
      end
  ...
  % record the best array details
 [maxpower ,maxindex]=max(abs([fitness.raw]));
 generation(iter_gen).sphereCoordinate(1,:)= arx(1:2:end,maxindex);
 generation(iter_gen).sphereCoordinate(2,:)= arx(2:2:end,maxindex);
 generation(iter_gen).ParrayW              = abs(fitness.raw(maxindex));
 disp(['Iteration=',num2str(iter_gen),':: best power=',num2str(maxpower),...
     '::Best position=',mat2str(round([generation(iter_gen).sphereCoordinate]))]);
 
 disp(['sum runtime=',num2str(Sum_runtime)]);
 iter_gen=iter_gen+1;
 if rem(round(Sum_runtime/3600),5)==0 && iter_gen> 2 || inopts.Buoy_Num==16
    save(['IPOP_CMAES_',inopts.siteName,'_NB_',num2str(inopts.Buoy_Num),'_id_',num2str(id),'.mat'],'generation')
 
 end
 if Sum_runtime >= 71* 3600
         disp ('runtime is finished') 
         return
 end
...
%             * (repmat(diagC,1,mu) .* arz(:,fitness.idxsel(1:mu)).^2 * weights);
  ...
  if sigma > inopts.UBounds
     sigma= inopts.UBounds;
  end
